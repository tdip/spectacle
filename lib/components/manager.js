  ...
  Manager.prototype._prevSlide = function _prevSlide() {
    ...
    if (this._checkFragments(this.props.route.slide, false) || this.props.route.params.indexOf("overview") !== -1) {
      if (slideIndex > 0) {
        ...
        localStorage.setItem("spectacle-slide", (0, _stringify2.default)({ slide: this._getHash(slideIndex - 1), forward: false, time: Date.now() }));
      }
    } else if (slideIndex > 0) {
      localStorage.setItem("spectacle-slide", (0, _stringify2.default)({ slide: this._getHash(slideIndex), forward: false, time: Date.now() }));
    }
  };
  ...
  Manager.prototype._nextSlide = function _nextSlide() {
    ...
    if (this._checkFragments(this.props.route.slide, true) || this.props.route.params.indexOf("overview") !== -1) {
      if (slideIndex < slideReference.length - 1) {
        this.context.history.replace("/" + (this._getHash(slideIndex + 1) + this._getSuffix()));
        localStorage.setItem("spectacle-slide", (0, _stringify2.default)({ slide: this._getHash(slideIndex + 1), forward: true, time: Date.now() }));
      }
    } else if (slideIndex < slideReference.length) {
      localStorage.setItem("spectacle-slide", (0, _stringify2.default)({ slide: this._getHash(slideIndex), forward: true, time: Date.now() }));
    }
  };
